...
import { AcceptancePolicy, BalanceScorer, CardValidator, Logger, MockLlmClient, PromptBuilder, runPipeline } from "@absurd/generator";
...
const runGenerate = async () => {
    ...
    const out = resolve(baseDir, getArg("out", "data/generation-log.jsonl") ?? "data/generation-log.jsonl");
    ...
    const constraints = "Fields required: id, version, name, type, faction, rarity, cost, stats, keywords, text, flavor, artPrompt, tags, balance, provenance.";
...
const runSim = async () => {
    const baseDir = resolve(fileURLToPath(new URL("../../..", import.meta.url)));
    const deckPath = resolve(baseDir, getArg("deck", "data/decks/starter.json") ?? "data/decks/starter.json");
