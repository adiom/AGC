...
export const createGameState = (playerA, playerB) => ({
    turn: 1,
...
export const drawCard = (state, playerId) => {
...
const findCardInHand = (player, cardId) => player.hand.findIndex((card) => card.id === cardId);
...
const addPermanent = (player, card) => {
    const permanent = {
        id: card.id,
        card,
        ownerId: player.id,
...
const resolveEffects = (state, playerId, actions, target) => {
    const player = state.players[playerId];
    if (!player)
        return { ok: false, reason: "player-not-found" };
    for (const action of actions) {
        switch (action.kind) {
            case "damage-creature": {
                ...
                if (targetId === "all") {
                    for (const unit of player.battlefield.concat(state.players[getOpponentId(state, playerId)].battlefield)) {
                        if (!unit.stats)
                            continue;
                    ...
                    const allPermanents = Object.values(state.players).flatMap((p) => p.battlefield);
...
export const playCard = (state, playerId, cardId, target) => {
    const player = state.players[playerId];
    if (!player)
        return { ok: false, reason: "player-not-found" };
...
export const attack = (state, attackerId, defender) => {
    const attackerOwner = Object.values(state.players).find((player) => player.battlefield.some((perm) => perm.id === attackerId));
    if (!attackerOwner)
    ...
    if (!attacker || !attacker.stats)
        return { ok: false, reason: "attacker-invalid" };
    if (attacker.summoningSickness)
        return { ok: false, reason: "summoning-sickness" };
    if (defender === "player") {
    ...
    const defenderOwner = Object.values(state.players).find((player) => player.battlefield.some((perm) => perm.id === defender));
    if (!defenderOwner)
        return { ok: false, reason: "defender-not-found" };
    const defending = defenderOwner.battlefield.find((perm) => perm.id === defender);
    if (!defending || !defending.stats)
        return { ok: false, reason: "defender-invalid" };
...
export const advancePhase = (state) => {
    const order = ["start", "draw", "main", "combat", "end"];
...
export const simulateRandomGame = (state, turns = 50) => {
    for (let t = 0; t < turns; t += 1) {
        ...
        if (player.hand.length > 0) {
            ...
            if (card)
                playCard(state, player.id, card.id);
        }
        state.phase = "combat";
...
const isCreature = (card) => card.type === "Creature" && card.stats;
const requiresTarget = (text) => {
    const lower = text.toLowerCase();
    if (lower.includes("target creature"))
        return "creature";
    if (lower.includes("target player"))
        return "player";
    return "none";
};
export const createGame = ({ deckA, deckB, seed }) => {
...
export const listLegalActions = (state) => {
    const player = state.players[state.activePlayerId];
    if (!player)
        return [];
    ...
    player.hand.forEach((card, handIndex) => {
        ...
        const allCreatures = Object.values(state.players).flatMap((p) => p.battlefield
    ...
    player.battlefield.forEach((perm, attackerIndex) => {
        if (!perm.stats || perm.summoningSickness)
            return;
...
export const getPublicView = (state, forPlayerId) => {
